package com.xf.wxf.realizationofmathematicalmodel.doodleview;
...
public class DoodleView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    private List<BaseAction> mBaseRevokes;
    ...
    public boolean back(Context context){
        if(mBaseActions != null && mBaseActions.size() > 0&&mBaseRevokes != null){
            Canvas canvas = mSurfaceHolder.lockCanvas();
            mBaseRevokes.add(mBaseActions.remove(mBaseActions.size() - 1));
            canvas.drawColor(Color.WHITE);
            for (BaseAction action : mBaseActions) {
                action.draw(canvas);
            }
     ...
     * @return 是否已经回退成功
     */
    public boolean revoke(Context context){
        if(mBaseActions != null &&mBaseRevokes != null && mBaseRevokes.size() > 0){
            mBaseActions.add(mBaseRevokes.remove(mBaseRevokes.size() - 1));
            Canvas canvas = mSurfaceHolder.lockCanvas();
            canvas.drawColor(Color.WHITE);
            for (BaseAction action : mBaseActions) {
                action.draw(canvas);
